...
class DCGan():
    ...
    def build(self):
        ...
        with tf.variable_scope('dis_conv1') as scope:
            self.conv1 = tf.nn.bias_add(tf.nn.conv2d(self.dinput, self.conv1_w, [1,2,2,1], 'SAME'), self.conv1_b)
            self.conv1m, self.conv1v = tf.nn.moments(self.conv1, [0,1,2])
            self.conv1b = tf.nn.batch_normalization(self.conv1, self.conv1m, self.conv1v,
                                                    self.conv1_sh, self.conv1_sc, self.eps)
            self.conv1r = 0.2 * self.conv1b + (1 - 0.2) * tf.nn.relu(self.conv1b)



        with tf.variable_scope('dis_conv2') as scope:
            self.conv2 = tf.nn.bias_add(tf.nn.conv2d(self.conv1r, self.conv2_w, [1,2,2,1], 'SAME') , self.conv2_b)
            self.conv2m, self.conv2v = tf.nn.moments(self.conv2, [0,1,2])
            self.conv2b = tf.nn.batch_normalization(self.conv2, self.conv2m, self.conv2v,
                                                self.conv2_sh, self.conv2_sc,self.eps)
            self.conv2r = 0.2 * self.conv2b + (1 - 0.2) * tf.nn.relu(self.conv2b)

        with tf.variable_scope('dis_conv3') as scope:
            self.conv3 = tf.nn.bias_add(tf.nn.conv2d(self.conv2r, self.conv3_w, [1,2,2,1], 'SAME'), self.conv3_b)
            self.conv3m, self.conv3v = tf.nn.moments(self.conv3, [0,1,2])
            self.conv3b = tf.nn.batch_normalization(self.conv3, self.conv3m, self.conv3v,
                                                    self.conv3_sh, self.conv3_sc, self.eps)
            self.conv3r = 0.2 * self.conv3b + (1 - 0.2) * tf.nn.relu(self.conv3b)

        with tf.variable_scope('dis_conv4') as scope:
            self.conv4 = tf.nn.bias_add(tf.nn.conv2d(self.conv3r, self.conv4_w, [1,2,2,1], 'SAME'), self.conv4_b)
            self.conv4m, self.conv4v = tf.nn.moments(self.conv4, [0,1,2])
            self.conv4b = tf.nn.batch_normalization(self.conv4, self.conv4m, self.conv4v,
                                                    self.conv4_sh, self.conv4_sc, self.eps)
            self.conv4r = 0.2 * self.conv4b + (1 - 0.2) * tf.nn.relu(self.conv4b)

        with tf.variable_scope('dis_fc') as scope:
            self.fcd_in = tf.reshape(self.conv4r, [-1,4*4*512])
            self.fcd = tf.nn.bias_add(tf.matmul(self.fcd_in, self.fcd_w), self.fcd_b)
        ...
        self.opt_dis = self.optimizer.minimize( 2 * self.loss, var_list = self.dis_param)
